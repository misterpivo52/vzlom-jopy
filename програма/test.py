...
def wait_for_file(filename, timeout=300):
    ...
    elapsed_time = 0
    while not xoxox(filename) and elapsed_time < timeout:
        print(f"Ожидаем, пока файл {filename} будет записан...")
        time.sleep(5)
        elapsed_time += 5
    if elapsed_time >= timeout:
        print(f"Ошибка: файл {filename} не был записан за {timeout} секунд.")
        return False
    return True
...
def main():
    ...
    print("Запуск основного цикла программы...")
